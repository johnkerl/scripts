...
def check_mul_matrix_dims(A, B, func_name):
	...
	[bnr, bnc] = B.dims()
	if (anc != bnr):
		print >> sys.stderr, "%s: mismatched mul lengths %dx%d, %dx%d." \
			% (func_name, anr, anc, bnr, bnc)
		sys.exit(1)
	return [anr, anc, bnr, bnc]

# ----------------------------------------------------------------
def check_matrix_times_vector_dims(A, v, func_name):
	[anr, anc] = A.dims()
	vn         = len(v)
	if (anc != vn):
		print >> sys.stderr, "%s: mismatched mul lengths %dx%d, %d." \
			% (func_name, anr, anc, vn)
		sys.exit(1)
	return [anr, anc]
...
def matrix_times_vector(A, v):
	Av = []
	[nr, nc] = check_matrix_times_vector_dims(A, v, "matrix_times_vector")
	for i in range(0, nr):
		Av.append(vecpair(A.elements[i], v))
	return Av
...
def vecpair(u, v):
	s = 0
	n = len(u)
	#n = check_same_list_dims(u, v, "sackmat vecpair")
	for i in range(0, n):
		ui = u[i]
		s += u[i] * v[i]
	return s
...
def vecnorm(u):
	uu = vecdot(u, u)
	return math.sqrt(abs(uu)) # abs in case it's complex.
def vecnormsq(u):
	return vecdot(u, u).real
def normalize(u):
...
# ----------------------------------------------------------------
def scalar_to_string(c, format="%9.6f"):
	if isinstance(c, complex):
		...
		if (ii < 0.0):
			return cnformat % (rr, abs(ii))
		else:
			return cpformat % (rr, abs(ii))
	else:
		return format % c
...
def print_row_vector(v, format="%9.6f"):
	n = len(v)
	for i in range(0, n):
		print scalar_to_string(v[i], format),
	print

def print_row_vector_no_cr(v, format="%9.6f"):
	n = len(v)
	for i in range(0, n):
		print scalar_to_string(v[i], format),

def print_row_vectors(vs, format="%9.6f"):
	nv = len(vs)
	for j in range(0, nv):
...
def read_row_vector(elt_scanner, file_name = "-"):
	...
	if (file_name == "-"):
		...
		try:
			...
			print >> sys.stderr, \
				"Couldn't open \"" + file_name + "\" for read."
			sys.exit(1)
...
#
# Sample syntax:
# A = sackmatc_m.paste_column_vectors([[1, 2], [3, 4], [5, 6]])
...
#
# Sample syntax:
# A = sackmatc_m.paste_row_vectors([[1, 2], [3, 4], [5, 6]])
...
# ----------------------------------------------------------------
# Sample syntax:
#   u = sackmatc_m.linear_combination([2, 3],    [[1,0,0],[0,0,1]])
#   v = sackmatc_m.linear_combination([2, 3, 4], [[1,0],[0,1],[100,200]])
# which results in
#   u =  [2.0, 0.0, 3.0]
#   v =  [402.0, 803.0]
...
class sackmat:
	...
	# B can be whatever as far as Python is concerned, but currently I support
	# only B being a matrix (sackmat object), vector (list), or scalar.
	# 
	...
	def printf(self, format="%9.6f"):
		...
		for i in range(0, nr):
			for j in range(0, nc):
				s = scalar_to_string(self.elements[i][j], format)
				print s,
			print
		return
	...
	# xxx doc self_start_col:  for when previous columns are already known to
	# be zero.
	...
	def householder_UT_pass_on_submatrix(self, submxidx, tol=1e-5, arg_Q=0.0):
		...
		# Get the 1st column of the submatrix; call it u.
		u = self.get_submatrix_column(submxidx, submxidx)

		# Compute ||u||, v with v[0] = +/- ||u||, and axis = u-v.
		normu = vecnorm(u)
		axis  = copy.copy(u)

		# xxx should be if *any* elements are complex
		if (isinstance(u[0], complex)):
			# Complex case (found on the web):
			# * v[0] = +/- e^{i theta} ||u|| where u[0] = r e^{i theta};
			# * axis = u - v.

			# This paragraph computes e^{i theta} from u[0],
			# then e^{i theta} ||u||.
			absu0 = abs(u[0])
			eitheta = 1.0
			if (absu0 != 0.0):
				eitheta = u[0]/absu0
			eithetanormu = eitheta * normu

			# Now, either add or subtract eithetanormu from u[0], depending on
			# which gives the least cancellation error.
			#
			# v[0] = +/- e^{i theta} ||u||
			# a[0] = u[0] - v[0] = -/+ e^{i theta} ||u||

			a0p = u[0] + eithetanormu
			a0n = u[0] - eithetanormu
			...
			v = [0] * len(u) # xxx debug only
			v[0] = v0        # xxx debug only

		else:
			# Real case:
			# * v[0] = +/- ||u||, with sign chosen to minimize cancellation
			#   in axis[0] = u[0] - v[0].
			# * axis = u - v.
			#
			# Note that only the first element of v is non-zero, so a full
			# vector subtract is unnecessary.
		...
		if 0:
			...
			print "  @@ Q^2="; Q2.printf(); print
			print "  @@ sign=", sign; print
			print
			print "  @@ eitheta=", eitheta
			print "  @@ u=",;  print_row_vector(u)
			print "  @@ v=",;  print_row_vector(v)
			print "  @@ a=",;  print_row_vector(axis)
			print
			print "  @@ Qu=",; print_row_vector(Qu)
			print "  @@ Qv=",; print_row_vector(Qv)
			print "  @@ Qa=",; print_row_vector(Qa)
			print
			print "  @@ Qu-Qv=",; print_row_vector(vecsub(Qu,Qv))
			print "  @@  u-Qv=",; print_row_vector(vecsub(u,Qv))
			print "  @@ Qu-v=",; print_row_vector(vecsub(Qu,v))
			print "  @@ Qv-u=",; print_row_vector(vecsub(Qv,u))
			print "  @@ Q(u-Qv)=",; print_row_vector(Q*vecsub(u,Qv))
			print "  @@ Qtr =",; print_row_vector(Qtr)
			print "  @@ Qtr times u=",; print_row_vector([vecpair(Qtr,u)])
			print
			print "  @@ || u||=", vecnorm(u)
			print "  @@ || v||=", vecnorm(v)
			print "  @@ || a||=", vecnorm(axis)
			print "  @@ ||Qu||=", vecnorm(Qu)
			print "  @@ ||Qv||=", vecnorm(Qv)
			print "  @@ ||Qa||=", vecnorm(Qa)
	...
	# ----------------------------------------------------------------
	# The sign return value is for callers that are tracking signs of
	# determinants of Householder matrices.
	def householder_UT(self, signlist=[1]):
		[nr, nc] = self.dims()
		num_HH = nr
		sign = 1
		if (nr > nc):
	...
	# Then
	#   R = Q A.
	# Orthogonal matrices Q (e.g. Householders) have Q^* Q = I so we may
	...
	def polar_decomp(self):
		...
		ApA = Astar * self # xxx Perhaps make an A^* A method.
		P = ApA.sqrt()
		U = self * P.inv()
		return [U, P]
	...
	def det(self):
		...
		# except in cases when a column is entirely zero.
		sign = A.householder_UT()

		# Take the product along the diagonal.  The sign accounts for the
	...
	def kernel_basis(self):
		...
		# data set which tripped it off.  I remember the problem was with large
		# numbers ... the check was using absolute instead of relative error.
		self.check_kernel_basis(kerbas, dimker)
	...
	def exp(self, tol=1e-12, maxits=165):
		...
		while (1):
			...
			if (k > maxits):
				print >> sys.stderr, \
					"sackmatc_m.exp:  max # iterations (%d) exceeded" \
					% (maxits)
				sys.exit(1)
	...
	def sqrt(self):
		...
		print >> sys.stderr, "sackmatc.sqrt:  maxiter (%d) exceeded." \
			% (maxiter)
		sys.exit(1)
...
def write_matrix(A, file_name, format="%9.6f"):
	...
	for i in range(0, nr):
		for j in range(0, nc):
			if (j > 0):
				file_handle.write(' ')
			file_handle.write(scalar_to_string(A.elements[i][j]))
		file_handle.write('\n')

	if (file_name != "-"):
		file_handle.close()

def write_matrix_as_column(A, file_name, format="%9.6f"):
	[nr, nc] = A.dims()
...
def gram_schmidt(A, tol = 1e-7):
	...
	for j in range(0, nr):
		...
		for k in range(0, j):
			...
			denom = vecdot(qk, qk)
			if (abs(denom) < tol):
				print >> sys.stderr, "Row ", k, \
					" of Q is zero (or near-zero) in sackmat_m.gram_schmidt."
				sys.exit(1)
			quot = numer / denom
